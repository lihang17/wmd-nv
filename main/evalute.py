...
def test_evalute(recommend_model, model, csv_table_name, num_epochs, save_path='../data/model',train=True):
    """
    ...
    if not os.path.exists(save_path):
        ...
        Max = -1
        for doc_name in os.listdir(save_path):
            if int(doc_name) > Max:
                Max = int(doc_name)
        save_path = os.path.join(save_path,str(Max+1))
    ...
    for epoch in range(num_epochs):
        ...
        if train:
            ...
            Model_history = model.fit([*train_instances_tuple], np.array(Para.train_labels), batch_size=Para.small_batch_size, epochs=2,
                      verbose=0,
                      shuffle=True,
                      validation_split=0.33)
            #画出loss曲线图
            #plot_learning_curves(Model_history,'loss',len(Model_history.epoch),0,0.5)
        ...
        print('test,done!')
        filename = 'epoch{}_neighbor{}'.format(epoch,Para.neighbors)
        ...
        evaluate_path = os.path.join(save_path,'evaluate_result.csv')
        summary(evaluate_path, csv_table_name, evaluate_result, Para.topKs)
...
def load_model_evalute(recommend_model, model_path, csv_table_name, num_epochs,load_model_evaluate='../data/model_loaded_evaluate.csv'):
    """
    对test_mashup_id_list，test_api_id_list进行测试
    :param para类对象
    :param recommend_model: 可以是text_tag,CF,MF,only_MLP
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(test_mashup_num):
            ...
            for j in range(num):
                ...
                batch_api_ids = candidate_ids[j * test_batch_size:stop_index]
                batch_instances_tuple = recommend_model.get_instances(
                    Para.test_mashup_id_list[i][j * test_batch_size:stop_index], batch_api_ids)  # 多态
        ...
        summary(load_model_evaluate, csv_table_name, evaluate_result, Para.topKs)
...
def plot_learning_curves(history, label, epoches, min_value, max_value):
    data = {}
    data[label] = history.history[label]
    data['val_' + label] = history.history['val_' + label]
    pd.DataFrame(data).plot(figsize=(8, 5))
    plt.grid(True)
    plt.axis([0, epoches, min_value, max_value])
    plt.show()